...
export default function Edit({ attributes, setAttributes }) {
	...
	const blockProps = useBlockProps({ className: "calla-recharge-gallery" });
	...
	useEffect(() => {
		if (!Array.isArray(images) || images.length === 0) {
			setAttributes({ images: cloneDefaultImages() });
		}
	...
	return (
		<>
			...
			<section {...blockProps}>
				<div className="calla-recharge-gallery__inner">
					<div className="calla-recharge-gallery__grid">
						{galleryImages.map((image, index) => (
							<div
								key={`gallery-preview-${index}`}
								className={`calla-recharge-gallery__item${
									index === 0 ? " calla-recharge-gallery__item--feature" : ""
								}`}
							>
								<GalleryItem
									url={image.url}
